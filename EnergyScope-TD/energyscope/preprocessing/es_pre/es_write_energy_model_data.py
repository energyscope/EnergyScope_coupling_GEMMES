...
def print_data(config):
    ...
    if config['printing']:
        ...
        share_private_motorcycle_max = config['all_data']['Misc']['share_private_motorcycle_max']
        ...
        export_capacity = config['all_data']['Misc']['export_capacity']
        # [GW] Maximum power of electrical interconnections

        # Storage daily
        STORAGE_DAILY = config['all_data']['Misc']['STORAGE_DAILY']
        
        carbon_price = config['all_data']['Misc']['carbon_price']
        ...
        EXPORT_E_FUELS = list(resources.loc[resources['Category'] == 'Export_e_fuels', :].index)
        ...
        print_set(EXPORT_E_FUELS, 'EXPORT_E_FUELS', out_path)
        newline(out_path)
        n = 0
        ...
        print_set(['TS_COAL_STOVE'], 'TS_OF_DEC_TECH ["COAL_STOVE"]', out_path)
        ...
        print_param('export_capacity', export_capacity, '', out_path)
        newline(out_path)
        with open(out_path, mode='a', newline='') as file:
            writer = csv.writer(file, delimiter='\t', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
        ...
        newline(out_path)
        print_param('share_private_motorcycle_max', share_private_motorcycle_max, '', out_path)
        newline(out_path)
        ...
        newline(out_path)
        print_param('carbon_price', carbon_price, '', out_path)
