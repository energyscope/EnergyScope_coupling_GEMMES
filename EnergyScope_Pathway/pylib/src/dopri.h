...
public:	
	...
	// sets all parameters
		dopri(T atolIn, T rtolIn, T facIn, T facMinIn, T facMaxIn, int nStepMaxIn, T hInitIn, T hMinIn, T hMaxIn) : 
	...
	void solveLastPoint(const T* yInit, T* parms, T* out) override {
		...
		for (int it=1; it<nt; it++) { // it would be possible to get rid of this loop if we were not using events to make this function even faster
			...
			#endif
			// Use dopri method to estimate y at t+h
			dopriOneStep(t, y, parms, k0, k1, k2, k3, k4, k5, k6, k7, x, y, h, tol, yTemp, y0, y4thOrder); // y appears twice as an argument (one tells EulerOneStep where to take initial estimate of y and the other where to write the new value of y) because we do not need to store the full trajectory here.
		}
		// Complete out at last point
...
protected: 
	...
	void dopriOneStep(T& t, const T* y, const T* parms, T* k0, T* k1, T* k2, T* k3, T* k4, T* k5, T* k6, T* k7, T* x, T* out,
			  T& h, T* tol, T* yTemp, T* y0, T* y4thOrder) {
		// INITIALIZATION
