...
subject to nettotalcost_cal:
	NetTotalCost = TotalCost - Export_revenues;

# [Eq. 2.1]
subject to totalcost_cal:
	TotalCost = sum {j in TECHNOLOGIES} (tau [j]  * C_inv [j] + C_maint [j]) + sum {i in RESOURCES} C_op [i] + TotalGWP * carbon_price;
...
subject to export_revenues_calc:
	Export_revenues = 0;
...
# Can choose between TotalGWP and NetTotalCost
minimize obj: NetTotalCost;
