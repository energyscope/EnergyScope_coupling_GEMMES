...
subject to end_uses_t {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [l, h, td] = (if l == "ELECTRICITY" 
		...
		else (if l == "COLD_HIGH_T_SC" then
			(end_uses_input["COLD_HIGH_T_SC"] * cooling_time_series [h, td] / t_op [h, td])
...
# Peak in cooling	
subject to peak_highT_sc {j in TECHNOLOGIES_OF_END_USES_TYPE["COLD_HIGH_T_SC"], h in HOURS, td in TYPICAL_DAYS}:
